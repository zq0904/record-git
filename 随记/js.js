...
// 构造函数|函数(非箭头函数)|class类 中都有一个属性prototype 叫原型 这个属性也是一个对象 (开发者用）
// 所有实例对象都有一个属性__proto__ 这个属性和构造函数的原型(prototype)的指向相同  （浏览器用的原型__proto__ 不标准 开发不用 可以使用Object.getPrototypeOf()来获取一个对象的原型）
// 原型中的所有属性方法 都被 实例对象 共享
// 实例对象的__proto__ 指向 自身构造函数的prototype 最终指向 Object的prototype 指向 null
...
/^(?!\d+$)(?![a-zA-Z]+$)[\dA-Za-z_]{3}$/ // (?![0-9]+$) 预测该位置后面不全是数字 (?![a-zA-Z]+$) 预测该位置后面不全是字母 （可以包含数字、字母、下划线 且同时含有数字和字母 且长度3位）
...
// 先拉git pull  提示信息（ esc shift+:一起按 输入wq ）在提交 git push ！！！
